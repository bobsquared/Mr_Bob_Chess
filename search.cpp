...
const int lateMoveMargin[2][7] = {{0, 6, 13, 21, 30, 40, 51}, {0, 10, 22, 36, 52, 68, 86}};                /**< Margins for late move pruning in pvSearch*/
...
int pvSearch(Bitboard &b, ThreadSearch *th, int depth, int alpha, int beta, bool canNullMove, int ply) {
    ...
    if (singMove == NO_MOVE) {
        hashed = tt->probeTT(posKey, hashedBoard, depth, ttRet, alpha, beta, ply);
    ...
    if (!isPv && !isCheck && singMove == NO_MOVE && staticEval - 115 * depth + (45 * depth * improving) >= beta && std::abs(beta) < MATE_VALUE_MAX) {
        return staticEval;
    }
    ...
    while (moveList.get_next_move(move)) {
        ...
        if (numMoves > 0 && ret > -MATE_VALUE_MAX) {
            if (isQuiet) {
                ...
                // Late move pruning
                if (!isPv && depth <= 6 && quietsSearched > lateMoveMargin[improving][depth]) {
                    break;
                }

                // History move pruning
                if (th->history[b.getSideToMove()][moveFrom][moveTo] < depth * depth * (-350 - (725 * improving))){
                    continue;
                }
            }
        ...
        if (depth >= 8 && !extension && hashedBoard.move == move && hashedBoard.flag == 1 && hashedBoard.depth >= depth - 2 && std::abs(hashedBoard.score) < MATE_VALUE_MAX) {
            int singVal = hashedBoard.score - 2 * depth;
            th->searchStack[ply].singMove = move;
            score = pvSearch(b, th, depth / (isPv? 2 : 3), singVal - 1, singVal, false, ply);
        ...
        // Late move reductions
            ...
            lmr -= std::max(-2, std::min(2, (th->history[!b.getSideToMove()][moveFrom][moveTo] + cmh) / 2500));
    ...
    if (numMoves == 0) {
        return singMove == NO_MOVE? (isCheck? -MATE_VALUE + ply : 0) : alpha;
    }
    ...
    if (alpha >= beta && ((bestMove & (CAPTURE_FLAG | PROMOTION_FLAG)) == 0) && singMove == NO_MOVE) {
        ...
        int hist = th->history[b.getSideToMove()][bestMoveFrom][bestMoveTo] * std::min(3 * depth, 57) / 64;
        th->history[b.getSideToMove()][bestMoveFrom][bestMoveTo] += 64 * (depth * depth) - hist;

        if (prevMove != NULL_MOVE) {
            hist = th->counterHistory[b.getSideToMove()][prevPiece][prevMoveTo][piece][bestMoveTo] * std::min(3 * depth, 57) / 64;
            th->counterHistory[b.getSideToMove()][prevPiece][prevMoveTo][piece][bestMoveTo] += 64 * (depth * depth) - hist;
        }
        ...
        for (int i = 0; i < quietsSearched; i++) {
            ...
            hist = th->history[b.getSideToMove()][quietFrom][quietTo] * std::min(3 * depth, 57) / 64;
            th->history[b.getSideToMove()][quietFrom][quietTo] += 64 * -(depth * depth) - hist;

            if (prevMove != NULL_MOVE) {
                hist = th->counterHistory[b.getSideToMove()][prevPiece][prevMoveTo][piece][quietTo] * std::min(3 * depth, 57) / 64;
                th->counterHistory[b.getSideToMove()][prevPiece][prevMoveTo][piece][quietTo] += 64 * (-depth * depth) - hist;
            }
        }
    }
...
BestMoveInfo pvSearchRoot(Bitboard &b, ThreadSearch *th, int depth, MoveList moveList, int alpha, int beta, bool analysis, int id) {
    ...
    while (moveList.get_next_move(move)) {
        ...
        // Late move reductions
            ...
            lmr--;
            lmr -= std::max(-2, std::min(2, (th->history[!b.getSideToMove()][get_move_from(move)][get_move_to(move)]) / 1500));
